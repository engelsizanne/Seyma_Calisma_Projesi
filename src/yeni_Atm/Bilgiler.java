...
public class Bilgiler {
    ...
    private static void sifreGir() {
        ...
        if (sifre.equals(sifrem)) {
            ...
            if (select == 1) {
                System.out.println("XXX Birden fazla hatali giris oldugu icin kartiniz bloke olmustur. XXXX");
            } else {
                hataliSifre();
                sifreGir();
            }
    ...
    private static void hataliSifre() {
        System.out.println("xxx Şifreniz Hatalıdır. Tekrar Giriş Yapın. xxx");
        select--;
        System.out.println("Kalan Hakkınız : " + select);
    }

    public static void kartDogruMu(String kartNumarası) {
        kartNumarası = kartNumarası.replace(" ", "");
        if (kartNumarası.length() == 16) {
        } else {
        }
    }
    ...
    public static void islemYapma(int secim) {
        switch (secim) {
            ...
            case 6:
                ...
                break;
            default:
                System.out.println(" xxx Hatali tuslama yaptiniz! xxx");
                ekran();
